...
#Region "--------------------------|  UI IMAGES             |-------------------------- GOOD"
    ...
    Dim bmp_error, bmp_star, icoMusic32, icoMusic16, icoFolder16, icoFolder32 As Bitmap
    Public Sub LoadUIImages()
        bmp_error = New Bitmap(Application.StartupPath + "\err.png")
        bmp_star = New Bitmap(Application.StartupPath + "\star_gray.png")
        icoMusic32 = New Bitmap(Application.StartupPath + "\muz.png")
        icoMusic16 = New Bitmap(50, 16) 'Application.StartupPath + "\muz16.png")
        icoFolder16 = New Bitmap(Application.StartupPath + "\dir_16.png")
        icoFolder32 = New Bitmap(Application.StartupPath + "\dir_32.png")
...
#Region "------|  Sortings  |-"
    ...
    Private Sub SwapImages(ByVal a As Long, ByVal b As Long)
        ...
        If a = Selected Then Selected = b Else If b = Selected Then Selected = a
    End Sub
#End Region
#End Region
...
#Region "--------------------------|  THUMBNAILS            |--------------------------"
    ...
    Private Sub MakeThumbnail(ByVal FileName As String)
        If Wire.Y >= 32 Then
            LoadedThumbnail = icoMusic32.Clone
        Else
            LoadedThumbnail = icoMusic16.Clone
        End If
    End Sub
#End Region
#Region "-----|  Resizing "
...
#Region "-----|  Draggings"
    ...
    Sub StartDragDrop(ByVal i As Integer)
        ...
        If Image(i).Selected = True Then
            ...
            Me.DoDragDrop(m, DragDropEffects.Copy)
        Else
            Dim str(0) As String
        ...
        End If
    End Sub
    Private Sub picMain_DragEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Me.DragEnter
        e.Effect = DragDropEffects.Copy
        If IsSmthDragging = True Then
            Cl.X = e.X - Me.Left - Me.Parent.Left
            Cl.Y = e.Y - Me.Top - Me.Parent.Top
            If Not (Image(ChosenObj).X <= Cl.X - Canvas.X _
            And Image(ChosenObj).X + Image(ChosenObj).WidthWithText >= Cl.X - Canvas.X _
            And Image(ChosenObj).Y <= Cl.Y - Canvas.Y _
            And Image(ChosenObj).Y + Wire.Y >= Cl.Y - Canvas.Y) Then
                Image(ChosenObj).X = Cl.X - Image(ChosenObj).WidthWithText / 2 - Canvas.X
                Image(ChosenObj).DestX = Cl.X - Image(ChosenObj).WidthWithText / 2
                Image(ChosenObj).Y = Cl.Y - Wire.Y / 2 - Canvas.Y
    ...
    Private Sub ucImagesBox_DragOver(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Me.DragOver
        ...
        If IsSmthDragging = True Then
            ...
            If Sorting = "user" And NDraggingFiles = 1 And Image(ChosenObj).Selected = False Then
                Dim ii As Short
                If Arrangement = "gorizontal" Then
                    x = x - Canvas.Plus - Canvas.X : y = y - Wire.dY - Canvas.Y
                    ii = (Canvas.Columns * Math.Truncate(y / (Wire.Y + Wire.dY)) + Math.Truncate(x / (Wire.X + Wire.dX)) + 1)
                Else
                    y = y - Canvas.Plus - Canvas.Y : x = x - Wire.dX - Canvas.X
        ...
        End If
    End Sub
    Private Sub picMain_DragDrop(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Me.DragDrop
        Dim files() As String = e.Data.GetData(DataFormats.FileDrop, True)
        Dim t As String
        StopLoading = False
        For Each t In files
    ...
    End Sub
#End Region
    Dim LeftMouseButton As Boolean = False
    Dim RightMouseButton As Boolean = False
#Region "-----|  Mouse Events"
    ...
    Private Sub picPhoto_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseDown
        'If e.Button = Windows.Forms.MouseButtons.Right Then MDTime = 1
        'If Allow Then
        If e.Button = Windows.Forms.MouseButtons.Left Then LeftMouseButton = True ': txt1.Text += vbNewLine + "left down"
        If e.Button = Windows.Forms.MouseButtons.Right Then RightMouseButton = True ': txt1.Text += vbNewLine + "right down"
        FirstX = e.X
        FirstY = e.Y
            ...
            If e.X >= a.X And e.X <= a.X + a.Image.Width And e.Y >= a.Y And e.Y <= a.Y + a.Image.Height And a.Visible = True Then
                Select Case i
                    ...
                    Case 6
                        OrderImages()
                    Case 9
                        FileTags.Tags(Image(Selected).InTagsIngex).Rating = 1
                    Case 10
                        FileTags.Tags(Image(Selected).InTagsIngex).Rating = 2
                    Case 11
                        FileTags.Tags(Image(Selected).InTagsIngex).Rating = 3
                End Select
                ChosenObj = -i - 1
                Exit For
            End If
    ...
    Private Sub picPhoto_MouseMove(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseMove 'Me.MouseMove
        'If Allow Then
        If e.Button <> Windows.Forms.MouseButtons.None And (FirstX <> e.X Or FirstY <> e.Y) Then
            If moved = False Then
                If (FirstX - e.X) ^ 2 + (FirstY - e.Y) ^ 2 >= 9 Then
                    moved = True
    ...
    Private Sub picPhoto_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseUp 'Me.MouseUp
        'If Allow Then
        If LeftMouseButton Or RightMouseButton Then
            ...
            End If
            If moved = False And e.Button = Windows.Forms.MouseButtons.Left And Not RightButtonPressedByPonter And Not moved Then 'And ChosenObj > 0 Then
                If ChosenObj > 0 Then
                    'RAISEEE!!
                    'Process.Start(Image(Selected).FileName, AppWinStyle.NormalFocus)
                    FileTags.Tags(Image(Selected).InTagsIngex).LaunchingTimes += 1
                Else
                    For i As Long = 1 To NImages
                        Image(i).Selected = False
                    Next
                End If
            ElseIf moved = False And (e.Button = Windows.Forms.MouseButtons.Right Or RightButtonPressedByPonter) Then
                If ChosenObj > 0 Then
                    Image(Selected).Selected = Not Image(Selected).Selected
                Else
    ...
    Private Sub ucImagesBox_MouseWheel(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseWheel
        If Arrangement = "gorizontal" Then
            ...
            End If
            If Image(Selected).Y + Canvas.Y < 0 Then
                Selected = Math.Truncate((-Canvas.Y) / (Wire.Y + Wire.dY)) * Canvas.Columns + Canvas.Columns + 1
                If Selected <= 0 Then Selected = 1
                If Selected > NImages Then Selected = NImages
            End If
            If Image(Selected).Y + Canvas.Y > Me.Height - Wire.Y Then
                Selected = Math.Truncate((-Canvas.Y) / (Wire.Y + Wire.dY) + Canvas.LinesInBox) * Canvas.Columns
                If Selected <= 0 Then Selected = 1
                If Selected > NImages Then Selected = NImages
            End If
        Else
            ...
            End If
            If Image(Selected).X + Canvas.X < 0 Then
                Selected = Math.Truncate((-Canvas.X) / (Wire.X + Wire.dX)) * Canvas.Columns + Canvas.Columns + 1
                If Selected <= 0 Then Selected = 1
                If Selected > NImages Then Selected = NImages
            End If
            If Image(Selected).X + Canvas.X > Me.Width - Wire.X Then
                Selected = Math.Truncate((-Canvas.X) / (Wire.X + Wire.dX) + Canvas.LinesInBox) * Canvas.Columns
                If Selected <= 0 Then Selected = 1
                If Selected > NImages Then Selected = NImages
            End If
        End If
        'NextFrame(True)
    End Sub
    Private Sub picMain_MouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseDoubleClick
    ...
    Private Sub ChooseObj(ByVal key As Keys)
        Select Case key
            ...
            Case Keys.Home
                Selected = 1
            Case Keys.PageDown
                Selected += Canvas.LinesInBox * Canvas.Columns
            Case Keys.PageUp
                Selected -= Canvas.LinesInBox * Canvas.Columns
            Case Keys.Down
                If Arrangement <> "gorizontal" Then
    ...
    Private Sub ucImagesBox_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        If InputLine.Length > 0 Then
            If InputLine = "АRENAME" Then
                Dim str As String
                Dim old_str As String = Image(Selected).FileName
                ...
                If str <> "" Then
                    Try
                        ...
                        Image(Selected).FileName = Mid(old_str, 1, old_str.Length - IO.Path.GetFileName(old_str).Length) + str
                        FileTags.Files(Image(Selected).InTagsIngex) = Image(Selected).FileName
                    Catch ex As Exception
            ...
            Else
                ...
                Else
                    ...
                    Else
                        For i As Long = 1 To NImages
                            If InStr(LCase(IO.Path.GetFileName(Image(i).FileName)), str) > 0 Then Selected = i : Exit For
                        Next
                    End If
                End If
            End If
        ...
        If e.Control = False And e.Alt = False And e.Shift = False Then
            ...
            Select Case e.KeyCode
                ...
                Case Keys.Enter
                    If Not IsEnterDown Then
                        IsEnterDown = True
                        If Selected >= 1 And Selected <= NImages Then
                            'RAISEEVENT!!!
                            'Process.Start(Image(Selected).FileName, AppWinStyle.NormalFocus)
                            FileTags.Tags(Image(Selected).InTagsIngex).LaunchingTimes += 1
                        End If
            ...
            End Select
        End If
        If e.KeyCode = Keys.ShiftKey Or e.KeyCode = 18 Then StartSelectionFrom = Selected
        'MsgBox(e.KeyCode)
        If e.Shift Then
            If e.KeyCode = Keys.Up Or e.KeyCode = Keys.Down Or e.KeyCode = Keys.Left Or e.KeyCode = Keys.Right Then
                ChooseObj(e.KeyCode)
                Dim st As Short = 1
                If StartSelectionFrom > Selected Then st = -1
                For i As Long = StartSelectionFrom To Selected Step st
        ...
        If e.Control Then
            Select Case e.KeyCode
                Case Keys.D0
                    FileTags.Tags(Image(Selected).InTagsIngex).Rating = 0
                    Image(Selected).ReLoaded = False
                    IsNotEverithingLoaded = True
                Case Keys.D1
                    FileTags.Tags(Image(Selected).InTagsIngex).Rating = 1
                    Image(Selected).ReLoaded = False
    ...
    Public Sub SetCanvas()
        If Arrangement = "gorizontal" Then
            If Canvas.LinesInBox >= 3 Then
                If Image(Selected).Y < -Canvas.Y + (Wire.Y + Wire.dY) Then
                    Canvas.DestY = -Image(Selected).Y + Wire.dY + (Wire.Y + Wire.dY)
                End If
                If Image(Selected).Y > -Canvas.Y + Me.Height - (Wire.Y + Wire.dY) * 2 Then
                    Canvas.DestY = -Image(Selected).Y + Me.Height - (Wire.Y + Wire.dY) * 2
                End If
            Else
                If Image(Selected).Y < -Canvas.Y Then
                    Canvas.DestY = -Image(Selected).Y + Wire.dY
                End If
                If Image(Selected).Y > -Canvas.Y + Me.Height - (Wire.Y + Wire.dY) Then
                    Canvas.DestY = -Image(Selected).Y + Me.Height - (Wire.Y + Wire.dY)
                End If
            End If
            If Canvas.DestY > 0 Then Canvas.DestY = 0
            If Canvas.DestY < Canvas.MinY Then Canvas.DestY = Canvas.MinY
        ...
        Else
            If Canvas.LinesInBox >= 3 Then
                If Image(Selected).X < -Canvas.X + (Wire.X + Wire.dX) Then
                    Canvas.DestX = -Image(Selected).X + Wire.dX + (Wire.X + Wire.dX)
                End If
                If Image(Selected).X > -Canvas.X + Me.Width - (Wire.X + Wire.dX) * 2 Then
                    Canvas.DestX = -Image(Selected).X + Me.Width - (Wire.X + Wire.dX) * 2
                End If
            Else
                If Image(Selected).X < -Canvas.X Then
                    Canvas.DestX = -Image(Selected).X + Wire.dX
                End If
                If Image(Selected).X > -Canvas.X + Me.Width - (Wire.X + Wire.dX) Then
                    Canvas.DestX = -Image(Selected).X + Me.Width - (Wire.X + Wire.dX)
                End If
            End If
            If Canvas.DestX > 0 Then Canvas.DestX = 0
            If Canvas.DestX < Canvas.MinX Then Canvas.DestX = Canvas.MinX
...
#Region "-----|  Drawing"
    ...
    Public Sub Draw_Picturies()
        If BmpMain.Height > 5 Then
            ...
            GraphicsMain.TextRenderingHint = Drawing.Text.TextRenderingHint.ClearTypeGridFit
            Dim I As Long = Selected
            Dim x, y As Long
            Dim II As Long
            For II = NImages To 1 Step -1
                If II <> Selected And Image(II).Y + Canvas.Y > -Wire.Y And Image(II).Y + Canvas.Y < Me.Height And Image(II).X + Canvas.X > -Wire.X And Image(II).X + Canvas.X < Me.Width Then
            ...
            If Me.Focused And I > 0 And I <= NImages Then
                ...
                If (IsMouseDown And Selected = ChosenObj) Or IsEnterDown Then
    ...
    Private Sub DrawSimpleText(ByVal ii As Integer)
        ...
        If Image(ii).Loaded Then
            ...
            If Wire.Y < 40 Then x = Image(ii).X + Wire.Y + 2 + Canvas.X
        Else
            x = Image(ii).X + Canvas.X
        End If
...
#End Region
    ...
    Public Sub NextFrame(ByRef DrawInAnyWay)
        ...
        If (RightButtonCircleRadius > 0 Or DrawInAnyWay Or IsSmthDragging Or IsMouseDown Or Canvas.V <> 0 Or Canvas.VX <> 0 Or IsAnimatedImages = True Or Canvas.Y > 0 Or Canvas.Y < Canvas.MinY Or Canvas.X > 0 Or Canvas.X < Canvas.MinX Or Canvas.Animate) Then
            Draw_Picturies()
            ...
            If MDTime > 0 Then
                If Not moved Then
                    If MDTime <> 20 Then
                        MDTime += 1
                        RightButtonCircleOpacity = MDTime * 6 - 6
                        RightButtonCircleLastDelta = RightButtonCircleRadius
                        RightButtonCircleRadius = Math.Abs(Math.Sin((MDTime) * Math.PI / 25) * 100)
                        RightButtonCircleLastDelta -= RightButtonCircleRadius
                    End If
                    If MDTime = 20 Then
                        RightButtonPressedByPonter = True
                        If ChosenObj = 0 Then ChosenObj = -1111
                ...
                End If
            End If
            If RightButtonCircleRadius > 0 Then
                If MDTime = 0 Then
                    RightButtonCircleRadius -= RightButtonCircleLastDelta
                    RightButtonCircleOpacity *= 0.85
                End If
                If RightButtonCircleRadius > 0 Then
                    GraphicsMain.SmoothingMode = Drawing2D.SmoothingMode.AntiAlias

                    Dim r As Rectangle
                    r = New Rectangle(Cl.X - RightButtonCircleRadius / 2 + 3, Cl.Y - RightButtonCircleRadius / 2 + 3, RightButtonCircleRadius - 6, RightButtonCircleRadius - 6)
                    ...
                    r = New Rectangle(Cl.X - RightButtonCircleRadius / 2, Cl.Y - RightButtonCircleRadius / 2, RightButtonCircleRadius, RightButtonCircleRadius)
                    Dim c As Pen
                    c = New Pen(Color.FromArgb(Math.Min(RightButtonCircleOpacity * 1.6, 255), 255, 255, 255), 4)
                    'If LeftMouseButton Then
                    '    c = New Pen(Color.FromArgb(Math.Min(RightButtonCircleOpacity * 1.6, 255), 0, 255, 255), 4)
                    'End If
                    'If RightMouseButton Then
                    '    c = New Pen(Color.FromArgb(Math.Min(RightButtonCircleOpacity * 1.6, 255), 255, 255, 0), 4)
                    'End If
                    GraphicsMain.DrawEllipse(c, r)

                    r = New Rectangle(Cl.X - RightButtonCircleRadius / 2 + 2, Cl.Y - RightButtonCircleRadius / 2 + 2, RightButtonCircleRadius - 4, RightButtonCircleRadius - 4)
    ...
    Private Sub tmrAnimation_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tmrAnimation.Tick
        ...
        End If
        'Me.BackgroundImage.Size = Me.Size
        Me.BackgroundImage = BmpMain
        Me.Refresh()
    End Sub
    ...
    Private Sub ucImagesBox_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.LostFocus
        ...
        NextFrame(True)
    End Sub
    Private Sub picMain_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.LostFocus
        If IsMouseDown And ChosenObj > 0 And ChosenObj <= NImages Then
            StartDragDrop(ChosenObj)
        End If
    End Sub
