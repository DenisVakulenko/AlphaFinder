...
Module mdlKeys
    ...
    Public Function SendHotKey(ByVal Key As Keys, Optional Ctrl As Boolean = False, Optional Alt As Boolean = False, Optional Shift As Boolean = False) As Boolean
        Dim tempList = New List(Of tagINPUT)

        If Ctrl Then tempList.Add(GetKeyDownStructure(ControlKey))
        If Shift Then tempList.Add(GetKeyDownStructure(ShiftKey))
        If Alt Then tempList.Add(GetKeyDownStructure(AltKey))

        tempList.Add(GetKeyDownStructure(CShort(Key)))
        tempList.Add(GetKeyUpStructure(CShort(Key)))

        If Alt Then tempList.Add(GetKeyUpStructure(AltKey))
        If Shift Then tempList.Add(GetKeyUpStructure(ShiftKey))
        If Ctrl Then tempList.Add(GetKeyUpStructure(ControlKey))

        Dim objSend() As tagINPUT = tempList.ToArray
        Dim nInput = CUInt(objSend.Length)
        Return nInput = NativeMethods.SendInput(CUInt(objSend.Length), objSend, Marshal.SizeOf(GetType(tagINPUT)))
    End Function
    ...
    Private Function GetKeyDownStructure(ByVal key As Short) As tagINPUT
        Dim retVal As tagINPUT = New tagINPUT With {.dwType = INPUTTYPE.KEYBOARD}
